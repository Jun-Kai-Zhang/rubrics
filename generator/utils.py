...
def select_best_responses(scored_data: List[Dict], n: int = 1) -> Dict:
...
def get_base_url() -> str:
    ...
    Resolution order: explicit ``set_base_url()`` value -> ``LITELLM_BASE_URL``
...
def set_base_url(url: str) -> None:
    """Set the default API base URL and clear cached clients."""
    global _base_url
    with _clients_lock:
        _base_url = url
        _clients.clear()
...
def _get_client(base_url: Optional[str] = None) -> OpenAI:
    ...
    Clients are created lazily and cached per URL in a thread-safe way.
    """
        ...
        if url not in _clients:
            ...
            if not api_key:
                raise RuntimeError(
                    ...
                    "Set it to your litellm proxy master key, e.g.: "
                    "export LITELLM_KEY=sk-litellm-master-key"
                )
            _clients[url] = OpenAI(
                api_key=api_key,
                base_url=url,
...
def generate_via_api(
    ...
    """Call an OpenAI-compatible chat completion endpoint and return the assistant's response.
    ...
    max_retries:
        How many attempts (total) before giving up.
    retry_base_delay_seconds:
        Base delay for exponential backoff between retries.
    base_url:
        Override the default base URL for this call.  Pass the vLLM
        server URL here when the model is served locally, or ``None``
        to use the default litellm proxy.
